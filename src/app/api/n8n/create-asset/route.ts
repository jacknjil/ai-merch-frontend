...
export async function POST(req: NextRequest) {
  try {
    ...
    // Optional: mix style into the prompt
    const fullPrompt = style
      ? `${promptRaw}\n\nStyle: ${style}`
      : promptRaw;

    // 🔹 IMPORTANT: do NOT send "response_format" here
    ...
    for (const image of images as any[]) {
      ...
      if (image.b64_json) {
        ...
        // Fallback: fetch the URL and convert to buffer
      ...
      const docRef = await addDoc(collection(db, "assets"), {
        ...
        source: "n8n", // mark that this came from your workflow
    ...
    console.error("Error in /api/n8n/create-asset:", err);
    return NextResponse.json(
